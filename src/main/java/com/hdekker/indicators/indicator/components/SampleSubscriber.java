...
import com.hdekker.indicators.indicator.IndicatoryFactory;
...
public interface SampleSubscriber<K extends IndicatorSampleData> {
	...
	public static <K extends IndicatorSampleData> SampleSubscriber<K> builder() {
		...
		return (tuple3)->{
			...
			Function<List<IndicatorDetails>, List<Tuple2<IndicatorDetails, Tuple2<MutableInternalStateHolder, Integer>>>> stateGetter = getIndicatorFnAndState.apply(tuple3.getT3());
			//reactive
			return tuple3.getT1().map(sample->{
				
				Function<List<Tuple2<IndicatorDetails, Tuple2<MutableInternalStateHolder, Integer>>>, List<Tuple2<IndicatorEvent, IndicatorDetails>>> sub = computeIndicatorsAndUpdateMatchingIndicatorState.apply(sample.getSecondaryKey()).apply(sample.getValue());
	...
	Function<String,
	Function<Double,
	Function<List<Tuple2<IndicatorDetails, Tuple2<MutableInternalStateHolder, Integer>>>,
			List<Tuple2<IndicatorEvent, IndicatorDetails>>>>> computeIndicatorsAndUpdateMatchingIndicatorState
			 = (skToUpdate) -> (newValue) -> (inds) -> {
				 ...
				 return inds.stream().map(ind->{
					Tuple2<Optional<IndicatorEvent>, IndicatorInternalState> res = IndicatoryFactory.getIndicator(ind.getT1().getIndicatorKey())
					 			.test(Tuples.of(0, newValue, ind.getT2().getT1().getState()));
	...
	Function<List<IndicatorDetails>,
				List<Tuple2<IndicatorDetails, Tuple2<MutableInternalStateHolder, Integer>>>>> getIndicatorFnAndState
