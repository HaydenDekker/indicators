...
public class RSITest {
	...
	public void rsiTransformProducesCorrectValueGivenStateAndInput() {
		...
		IndicatorTestSpec output = rsiIndicatorTransform.apply(new IndicatorTestSpec(0, newValue.get(0), LocalDateTime.now(), initialState));
		assertThat(output.getValue(), closeTo(100.0, 2.0));
		// assertThat(output.getT2().getState(), hasKey("rsi-fn-1-steps"));
		assertThat(output.getIndicatorAttributeState().getState(), hasKey("rsi-fn-1-" + RSI.aveGain));
		assertThat(output.getIndicatorAttributeState().getState(), hasKey("rsi-fn-1-" + RSI.aveLoss));
		assertThat(output.getIndicatorAttributeState().getState(), hasKey("rsi-fn-1-" + RSI.rsi));
		assertThat(output.getIndicatorAttributeState().getState(), hasKey("rsi-fn-1-" + RSI.value));
